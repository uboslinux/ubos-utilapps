...
my( $name, $passwd, $uid, $gid, $quota, $comment, $gcos, $dir, $shell, $expire ) = getpwnam( 'docroot' ) ;
...
if( 'deploy' eq $operation || 'undeploy' eq $operation ) {
    ...
    my $content = <<CONTENT;
# This file is automatically generated by docroot's provision.pl.
...
CONTENT
    ...
    foreach my $key ( sort keys %keyToAppConfigIds ) {
        ...
        $content .= 'command="/usr/share/docroot/bin/safe-rsync.pl ' . join( ' ', @ids ) . '" ' . $key . "\n";
    }
